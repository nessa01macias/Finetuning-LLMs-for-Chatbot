...
def main():
    ...
    try:
        ...
        model = AutoModelForCausalLM.from_pretrained("nessa01macias/gemma-2b_sustainability-qa", low_cpu_mem_usage=True, trust_remote_code=False, torch_dtype=torch.float16, device_map = 'auto')
        tokenizer = AutoTokenizer.from_pretrained("google/gemma-2b", trust_remote_code=False)
        model.to(device)

        print("------------This script is used on model gemma-2b ---------------")
