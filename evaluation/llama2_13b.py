...
def main():
    ...
    model = AutoModelForCausalLM.from_pretrained("nessa01macias/llama-2-13b_sustainability-qa", trust_remote_code=False, torch_dtype=torch.float16, device_map = 'auto')
    tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-2-13b-hf", trust_remote_code=False)
    model.to(device)

    print("Model and tokenizer loaded - llama2")
